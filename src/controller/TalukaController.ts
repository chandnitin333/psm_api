...
export class TalukaController {
    ...
    static async createTaluka(req, res, next) {
        try {
            ...
            let taluka: any = await createTaluka(name, parseInt(district_id));
    ...
    static async updateTaluka(req, res, next) {
        try {
            let data = req.body;
            let taluka = await updateTaluka(parseInt(data.id), { name: data.name, district_id: parseInt(data.district_id) });
